...
//													alert("你选择了日期~!"+pm);
	...
	var store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
							url : 'plreport.do?reqCode=queryPzReportData'
						}),
	...
	var tbar = new Ext.Toolbar({
				items : [{
							...
							handler : function() {
								...
								var pm = Ext.util.Format.date(m, 'Y-m');
								exportExcel('plreport.do?reqCode=exportPzExcel&pm='+pm);
						}
					}]
			});
	...
	function createPzReport() {
		...
		Ext.Msg.confirm(
				'请确认',
				'确认要生成预测月份为<span style="color:red"><b>'+pm+'</b></span>月份的分品种预测报表数据吗？<br>',
				function(btn, text) {
					if (btn == 'yes') {
						if (runMode == '0') {
						...
						showWaitMsg();
						Ext.Ajax.request({
							url : './plreport.do?reqCode=createPzReport',
							params : {
								predictionMonth : pm
							},
