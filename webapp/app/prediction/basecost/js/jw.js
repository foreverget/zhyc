...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
							url : 'basecost.do?reqCode=queryJwData'
						}),
	...
	var jw_file_form = new Ext.form.FormPanel({
		id : 'id_jw_file_form',
		name : 'id_jw_file_form',
	...
	var jw_excel_window = new Ext.Window({
		layout : 'fit',
		width : 380,
		height : 300,
		resizable : false,
		...
		buttons : [{
					...
					handler : function() {
						...
						jw_file_form.form.submit({
									url : 'basecost.do?reqCode=importJwExcel',
									waitTitle : '提示',
									method : 'POST',
									waitMsg : '正在处理数据,请稍候...',
									success : function(form, action) {
										var predictionMonth = jw_file_form.get('importJwMonth').getValue();
	...
	var tbar = new Ext.Toolbar({
				items : [{
							...
							handler : function() {
								...
								Ext.Msg.confirm(
										'请确认',
										'确认要导入预测月份为<span style="color:red"><b>'+pm+'</b></span>月份的机物料费用数据吗？<br>',
										function(btn, text) {
											if (btn == 'yes') {
												jw_file_form.get('importJwMonth').setValue(pm);
												jw_excel_window.show();
											}
										});
							}
						}]
	...
	var grid = new Ext.grid.GridPanel({
				...
				title : '<span class="commoncss">机物料费用数据 </span>',
				height : 500,
				id : 'id_grid_jw',
				autoScroll : true,
				frame : true,
